# Plan 10-02: Invoice Hooks

## Objective

Create React Query hooks for invoice CRUD operations, including listing, creating, updating, sending, and voiding invoices. Integrates with billing foundation from 10-01.

## Wave

**Wave 2** - Depends on Wave 1 (10-01: Billing Data Foundation)

## Dependencies

- 10-01: Billing types, constants, validations, helpers

## Must-Haves

- [ ] Invoice query keys factory
- [ ] useInvoices - List invoices with filters
- [ ] useInvoice - Single invoice with relations
- [ ] useInvoiceSummary - Dashboard stats
- [ ] useCreateInvoice - Create with line items
- [ ] useUpdateInvoice - Update draft invoices
- [ ] useSendInvoice - Mark as sent
- [ ] useVoidInvoice - Mark as void
- [ ] useClientInvoices - Client-specific invoices

## Tasks

### 1. Create invoice hooks (`apps/admin/src/hooks/use-invoices.ts`)
  ...
  return useMutation({
    mutationFn: async (data: CreateInvoiceData) => {
      ...
      const { data: seqResult, error: seqError } = await supabase
        .rpc('nextval', { seq_name: 'invoice_seq' })

      if (seqError) throw seqError

      const invoiceNumber = `INV-${(seqResult as number).toString().padStart(5, '0')}`
...
- [ ] Commit: `feat(billing): add invoice hooks - CRUD, send, void, summary`

## Notes

- Invoice number generation uses database sequence for uniqueness
- Only draft invoices can be edited or deleted
- Sending invoice marks as sent with timestamp and email
- Summary hook provides dashboard metrics
- Client invoices filter uses same hook with client_id filter
