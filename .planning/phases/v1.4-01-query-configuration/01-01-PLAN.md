...
must_haves:
  ...
  artifacts:
    ...
    - path: "apps/admin/src/config/app-config.ts"
      ...
      exports: ["APP_CONFIG", "BUILDER_MARKUP", "calculateClientPrice"]
  key_links:
...
Purpose: Eliminate hardcoded magic numbers scattered across the codebase. All cache timing values and business configuration should come from single source of truth files.
...
- VENDOR_MARKUP (15%) centralized - currently hardcoded in work-order.ts
...
- src/config/app-config.ts exists with BUILDER_MARKUP, FEATURES, PAGINATION, APP_CONFIG
