...
1. Add import at top:
```typescript
import { BUILDER_MARKUP, calculateClientPrice } from '@/config/app-config'
```
...
3. Replace any inline markup calculations with the helper function:

BEFORE (around lines 110 and 133):
```typescript
// Apply 20% markup
const price = basePrice * (1 + BUILDER_MARKUP)
```

AFTER:
```typescript
const price = calculateClientPrice(basePrice)
```
 ...
 * Allows clients to modify their service plan with live pricing updates.
...
```
  ...
  <done>
- plan-editor.tsx imports BUILDER_MARKUP and calculateClientPrice from config
- No local BUILDER_MARKUP constant defined in the component
- Pricing calculations use the centralized helper function
  </done>
</task>
