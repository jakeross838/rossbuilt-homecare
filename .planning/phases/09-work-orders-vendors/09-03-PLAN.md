...
## Objective
Create React Query hooks for work order CRUD operations, status transitions, and related queries. These hooks provide the data layer for all work order functionality.

## Why This Plan
Work orders need comprehensive data operations:
- List work orders with filters and pagination
- Create work orders (from recommendations or manually)
- Update work order details
- Transition work order status
- Assign vendors and internal staff
- Schedule work
- Complete work with cost tracking
- Query work orders by property, client, vendor
  ...
  return useMutation({
    mutationFn: async (
      ...
      if (seqError) {
        ...
        var workOrderNumber = `WO-${timestamp}`
      } else {
        var workOrderNumber = `WO-${seqData}`
      }
...
Add these RPC functions to handle vendor job counts. Create a migration file if they don't exist:
...
## Verification
- [ ] useWorkOrders fetches list with all filter options
- [ ] useWorkOrder fetches single work order with all relations
- [ ] usePropertyWorkOrders fetches work orders for a property
- [ ] useVendorWorkOrders fetches work orders for a vendor
- [ ] useCreateWorkOrder creates work order and links to recommendation
- [ ] useUpdateWorkOrder updates work order fields
- [ ] useUpdateWorkOrderStatus transitions status with timestamps
- [ ] useAssignVendor assigns vendor and updates status
- [ ] useCompleteWorkOrder calculates costs and marks complete
- [ ] useCancelWorkOrder cancels and resets recommendation
- [ ] useWorkOrderCounts returns counts by status
- [ ] All hooks invalidate appropriate caches
