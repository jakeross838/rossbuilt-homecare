...
- **13-02**: Used direct Resend API calls via fetch in edge function (no SDK for simplicity)
- **13-02**: HTML email templates use inline styles for maximum email client compatibility
- **13-02**: Plain text version auto-generated by stripping HTML tags via regex
- **13-02**: Notification tracking updates database on send success/failure
- **13-02**: Created `lib/email/` directory for email-related utilities and templates
- **13-04**: Notification components directory at `components/notifications/` for UI components
- **13-04**: NotificationBadge shows count with "99+" overflow for counts above 99
- **13-04**: NotificationItem uses type-based icons (inspection, work_order, payment, reminder, alert, message)
...
- RESEND_API_KEY needs to be set in Supabase Edge Function secrets (see apps/admin/src/lib/email/RESEND_SETUP.md)
...
**Email Edge Function Complete (13-02):**
- Resend email edge function with Resend API integration
- HTML email templates for inspections, invoices, work orders
- React Query hooks for sending templated emails
- Setup documentation with deployment instructions
...
**Notification UI Components Complete (13-04):**
- NotificationBadge component with unread count indicator
- NotificationItem for individual notification display
- NotificationDropdown for header notification center
- NotificationPreferences form for managing settings
...
- supabase/functions/send-email/index.ts - Resend email edge function
- apps/admin/src/lib/email/templates.ts - HTML email templates
- apps/admin/src/hooks/use-email.ts - Email sending hooks
- apps/admin/src/lib/email/RESEND_SETUP.md - Resend setup documentation
